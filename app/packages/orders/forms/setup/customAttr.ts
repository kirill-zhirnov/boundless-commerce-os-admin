...
interface IAttrs {
	title: string|null;
	key: string|null;
	type: TOrderAttrHtmlType|null;
	options: IOptions[];
	hint: string|null;
	sort: number|null;
}

export default class CustomAttrSetupForm extends Form<IAttrs, IOrderAttrsModel> {
	getRules() {
		return [
			...
			['title', 'isUnique', {
				field: 'title',
				row: this.record,
				model: this.getModel('orderAttrs')
			}],
			['key', 'validateKey'],
			['key', 'isUnique', {
				field: 'key',
	...
	async save() {
		if (!this.record) {
			this.record = (this.getModel('orderAttrs') as IOrderAttrsModelStatic).build();
		}
		...
		if (this.record.isNewRecord) {
			const [{sort}] = await this.getDb().sql<{sort: number}>('select coalesce(max(sort), 0) + 10 as sort  from order_attrs');
	...
	validateOptions(options: IOptionsInput[]|any) {
		if (![TOrderAttrHtmlType.dropdown, TOrderAttrHtmlType.checkbox].includes(this.attributes.type)) {
			return;
		}
	...
	async loadRecord(): Promise<IOrderAttrsModel> {
		return await (this.getModel('orderAttrs') as IOrderAttrsModelStatic).findException({
			where: {
				attr_id: this.pk
			}
		}) as IOrderAttrsModel;
